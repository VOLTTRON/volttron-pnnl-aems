...
- **When a credential changes**, run `./rotate-secrets.sh` (or `.\rotate-secrets.ps1`) — it applies the change to running containers before restarting them. Editing `.env.secrets` and restarting without rotating will cause auth failures on DB-backed services.
