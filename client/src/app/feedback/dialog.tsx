...
import { PreferencesContext } from "../components/providers";
...
interface IViewFeedbackProps {
  ...
  icon?: IconName;
  feedback: NonNullable<ReadFeedbacksQuery["readFeedbacks"]>[0] | undefined;
}
...
export function ViewFeedback({ open, setOpen, icon, feedback }: IViewFeedbackProps) {
  ...
  const [updateFeedback, { loading }] = useMutation(UpdateFeedbackDocument, {
    refetchQueries: [ReadFeedbacksDocument],
  });

  const { data: filesInfo } = useQuery(ReadFilesDocument, {
    variables: {
        ...
        {feedback && feedback.id ? (
          <div className={styles.panel}>
            <div className={styles.feedback}>
              ...
              {filesInfo?.readFiles && (feedback.files?.length ?? 0) > 0 && (
                <div>
                  <p>
                    <strong>Files:</strong>
                  </p>
                  <div className={styles.files}>
                    <FilePreviews files={filesInfo.readFiles} />
                  </div>
                </div>
              )}
            </div>
            ...
            <div className={styles.actions}>
              ...
              <div className={styles.time}>
                <span>Last updated on {feedback.updatedAt ? formatDate(feedback.updatedAt, preferences?.timezone) : "—"}</span>
                <span>Created on {feedback.createdAt ? formatDate(feedback.createdAt, preferences?.timezone) : "—"}</span>
              </div>
            </div>
          </div>
        ) : (
