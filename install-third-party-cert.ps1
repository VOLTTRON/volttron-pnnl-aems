#
# Install a third-party TLS certificate into the Skeleton Traefik proxy (Windows).
#
...
if (-not (Test-Path $ENV_FILE)) {
  Write-Err "$ENV_FILE not found -- run from the skeleton\ repo root."
  exit 2
}
if (-not (Test-Path $YAML_FILE)) {
  Write-Err "Expected file $YAML_FILE not found -- is this the Skeleton repo?"
  exit 2
}
