...
To install a TLS certificate issued by your own CA (public, private, or organization-internal), use the installer script from the repo root:
...
Do **not** drop certificate files directly into `docker/proxy/`. Only the specific YAML files declared in [`docker-compose.yml`](../docker-compose.yml) (`certs-traefik.yml`, `dynamic.yml`) are bind-mounted from this directory into the Traefik container; anything else placed here is invisible to the running proxy. The installer script handles the volume side of the layout for you.

Also set `CERT_RESOLVER=` (empty) in `.env` so Traefik does not attempt to obtain a Let's Encrypt certificate on top of your custom one.
...
The host directory `docker/proxy/` contains only the Traefik dynamic-config YAML files (`certs-traefik.yml`, `dynamic.yml`) — each is bind-mounted individually into `/etc/traefik/dynamic/` inside the container. Files placed in this directory outside those bind mounts are not visible to Traefik.
