...
import { Configurations, Holiday_type, Holidays, Schedules, Setpoints, Units } from "@prisma/client";
...
const createConfigurationDefault = (unit: Partial<Units>): DeepPartial<UnitsFull> => {
  ...
  const holidays = HolidayType.values.map((h) => ({
    label: h.label,
    type: enabled.includes(h.name) ? Holiday_type.Enabled : Holiday_type.Disabled,
  }));
  return merge(unit, {
    configuration: {
      label,
