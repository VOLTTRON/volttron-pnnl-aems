...
# FastAPI AEMS Edge Setup Script
# This script generates configuration files for the FastAPI-based AEMS platform
# It mirrors setup-volttron.sh but skips VOLTTRON-specific steps (platform_config, BACnet proxy, SSL)
...
# Function to generate NF driver configuration
generate_nf_driver_config() {
    local output_dir="$1"
    ...
    log_info "Generating NF driver configuration"
    log_info "  Gateway URL: ${gateway_address}"
    log_info "  Campus: ${campus}, Building: ${building}, Prefix: ${prefix}"
    log_info "  Number of devices: ${num_configs}"

    local nf_config_dir="${output_dir}/configs/nf-driver"
...
EOF
    ...
    if [[ $? -eq 0 ]]; then
        log_success "NF driver configuration generated at: ${nf_config_path}"
    else
...
# Historian DB settings for config generation (optional)
...
FASTAPI_LOCK_FILE="${SETUP_DIR}/.fastapi_setup_complete"

log_info "Starting AEMS Edge Setup (FastAPI Mode)"

# Check setup completion status
FASTAPI_COMPLETED=false

# Check if FastAPI setup is already completed
if [[ -f "${FASTAPI_LOCK_FILE}" ]]; then
    log_info "FastAPI setup lock file found at: ${FASTAPI_LOCK_FILE}"
    FASTAPI_COMPLETED=true
    log_info "FastAPI setup already completed - nothing to do"
fi

# Exit if FastAPI setup is already completed
if [[ "${FASTAPI_COMPLETED}" == "true" ]]; then
    log_success "FastAPI setup has already been completed. Nothing to do."
    log_info "To force re-run FastAPI setup: delete ${FASTAPI_LOCK_FILE}"
    exit 0
fi
...
log_info "=== Starting FastAPI Setup ==="
...
log_info "Executing configuration generation for FastAPI"
...
# Generate NF driver configuration
log_info "Generating NF driver configuration for FastAPI"
generate_nf_driver_config "${OUTPUT_DIR}" "${VOLTTRON_GATEWAY_ADDRESS}" \
    "${VOLTTRON_CAMPUS}" "${VOLTTRON_BUILDING}" "${VOLTTRON_PREFIX}" "${NUM_CONFIGS}"
if [[ $? -eq 0 ]]; then
    log_success "NF driver configuration generated successfully"
else
    log_error "Failed to generate NF driver configuration"
    exit 1
fi
...
# - platform_config.yml generation/manipulation
# - BACnet proxy configuration
# - BACnet interface configuration
# - SSL cert removal from platform_config.yml

# Create FastAPI completion lock file
log_info "Creating FastAPI setup completion lock file: ${FASTAPI_LOCK_FILE}"
# Ensure the directory exists
mkdir -p "$(dirname "${FASTAPI_LOCK_FILE}")"
echo "FastAPI setup completed on $(date)" > "${FASTAPI_LOCK_FILE}"
echo "Campus: ${VOLTTRON_CAMPUS}" >> "${FASTAPI_LOCK_FILE}"
echo "Building: ${VOLTTRON_BUILDING}" >> "${FASTAPI_LOCK_FILE}"
echo "Prefix: ${VOLTTRON_PREFIX}" >> "${FASTAPI_LOCK_FILE}"
echo "Gateway Address: ${VOLTTRON_GATEWAY_ADDRESS}" >> "${FASTAPI_LOCK_FILE}"
echo "Timezone: ${VOLTTRON_TIMEZONE}" >> "${FASTAPI_LOCK_FILE}"
echo "Number of Configs: ${NUM_CONFIGS}" >> "${FASTAPI_LOCK_FILE}"

if [[ $? -eq 0 ]]; then
    log_success "FastAPI setup completed and lock file created"
    FASTAPI_COMPLETED=true
else
    log_warning "Failed to create FastAPI lock file, but setup completed successfully"
    FASTAPI_COMPLETED=true
fi
...
log_success "FastAPI AEMS Edge setup process completed!"
log_info "Configuration files generated and copied to: ${OUTPUT_DIR}"
