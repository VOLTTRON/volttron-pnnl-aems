...
class OptimalStartZoneModel(ABC):
    ...
    def _setup(self, config: OptimalStartConfig, get_current_schedule_fn: Callable):
        self.cfg = config
    ...
    def load_model(self, model_dict: dict[str, any]):
        """
    ...
    def train(self, data: pd.DataFrame, prestart: int):
        """
    ...
    @abstractmethod
    def train_cooling(self, data: pd.DataFrame):
        pass

    @abstractmethod
    def train_heating(self, data: pd.DataFrame):
        pass

    def heat_transfer_rate(self, data: pd.DataFrame):
        df_list = []
...
class Carrier(OptimalStartZoneModel):
    ...
    def __init__(self, config: OptimalStartConfig, schedule_fn: Callable):
        super()._setup(config, schedule_fn)
        self.c1 = []
        self.h1 = []
        self.oat_clg = []
        self.oat_htg = []
    ...
    def calculate_prestart(self, data: pd.DataFrame):
        if data.empty:
