...
# Load the database password from the environment so psql can authenticate.
...
export PGPASSWORD="${HISTORIAN_DATABASE_PASSWORD:-${POSTGRES_PASSWORD:-}}"
if [ -z "${PGPASSWORD}" ]; then
    echo "ERROR: no database password available. Tried HISTORIAN_DATABASE_PASSWORD and POSTGRES_PASSWORD (both unset)." >&2
    echo "Hint: run this via ./repair-historian-replication.sh from aems-app/ so the host .env.secrets password is forwarded via docker exec -e." >&2
    exit 1
fi
...
# HISTORIAN_REPLICATOR_PASSWORD is forwarded by the host wrapper
# (repair-historian-replication.sh) via docker exec -e.
...
echo ""
echo "Recommended (resumable, safe over unreliable / cellular links):"
echo "    On each subscriber host, run:"
echo "        aems-app/subscribe-historian.sh --publisher-host <this-host>"
echo "    It creates the subscription with copy_data=false and backfills"
echo "    historical rows in resumable chunked transactions."
echo ""
echo "Fallback (lab / broadband only — single-transaction initial COPY):"
echo "    On each subscriber:"
echo "        ALTER SUBSCRIPTION historian_sub DISABLE;"
echo "        ALTER SUBSCRIPTION historian_sub SET (slot_name = NONE);"
echo "        DROP SUBSCRIPTION historian_sub;"
echo "        CREATE SUBSCRIPTION historian_sub CONNECTION '...' PUBLICATION historian_pub"
echo "          WITH (copy_data=false, create_slot=true, slot_name='historian_sub_slot');"
echo "    Then run subscribe-historian.sh to backfill historical rows."
echo "================================================"
