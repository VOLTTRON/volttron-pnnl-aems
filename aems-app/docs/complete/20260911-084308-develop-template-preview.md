...
### 20260914-101500 — Fix: template refresh short-circuited by setup lock file
- After removing `refresh-templates.sh`, the user reported the UI still showed the old room-type shape. Diagnosed via container inspection:
  - Host source: wrapped `"room-type": { "value": { "_type": "evaluate", … } }` (correct, edited 2026-09-11).
  - Shared bind mount at `./docker/volttron/setup/templates/`: still had the OLD unwrapped `"room-type": { "_type": "evaluate", … }` (from 2026-09-03).
  - `volttron-setup` container logs: `Volttron setup lock file found at: /home/user/setup/.setup_complete` → `Volttron setup has already been completed. Nothing to do.` — the script short-circuited before reaching the template copy step.
- Root cause: [setup-volttron.sh](../../../aems-edge/setup-volttron.sh) creates a `${VOLTTRON_LOCK_FILE}` in a docker-managed `volttron-setup` volume (docker-compose.yml:845) and guards the entire setup body behind that lock. The lock is only invalidated when the historian-secret fingerprint changes — template edits are opaque to it. Recreating the container doesn't help because the docker-managed volume persists across recreates, so the lock survives.
- **Fix** (following the existing "pre-lock self-heal chmod" precedent at [setup-volttron.sh:159-167](../../../aems-edge/setup-volttron.sh#L159-L167)): moved the template `rm -rf` + `cp -r` + `chmod -R a+rX` block **above** the lock check in [setup-volttron.sh](../../../aems-edge/setup-volttron.sh). Templates now refresh unconditionally on every setup-container run; the heavy Volttron-setup steps stay gated by the lock and only re-run on historian-secret rotation. Deleted the redundant copy from the guarded section.
- Updated the [aems-app/docker/CLAUDE.md](../../docker/CLAUDE.md) refresh bullet to describe the pre-lock block explicitly.
- Verified: rebuilt `volttron-setup`, force-recreated the container, log now shows `Refreshing templates directory from /home/user/configurations/templates` BEFORE the lock hit — and `docker exec aems-server cat /app/volttron/templates/criteria_config.json` now shows the wrapped form.
- Also cleaned up the stale lock on the running dev instance (`docker run --rm -v aems_volttron-setup:/setup alpine rm -f /setup/.setup_complete`) so the immediate render matches; going forward the lock never needs manual removal because template edits bypass it.

### 20260914-092543 — Rolled back refresh-templates.sh; use start-services.sh
- User feedback: an explicit refresh script is redundant with the existing `start-services.sh` entry point, and adding a direct host bind-mount for template files (my prior suggestion) would break the perms/UID/SELinux dance on Linux — the current pattern (bake into image → setup container copies into a docker-managed shared area with `chmod -R a+rX`) is the established convention, mirrored by [grafana-setup](../../docker/docker-compose.yml).
- `start-services.sh` already handles the refresh: `docker compose build` invalidates the `volttron-setup` image's `COPY .` layer, `docker compose up -d` sees the new image ID and recreates the exited setup container, and [setup-volttron.sh:387-397](../../../aems-edge/setup-volttron.sh#L387-L397) does an idempotent `rm -rf` + `cp -r` of templates on every run. `aems-server` and `aems-services` then read the fresh files on the next request / cron tick.
- Changes:
  - Deleted [aems-app/refresh-templates.sh](../../refresh-templates.sh) and [aems-app/refresh-templates.ps1](../../refresh-templates.ps1).
  - Rewrote the "Refresh ILC configuration templates" bullet in [aems-app/docker/CLAUDE.md](../../docker/CLAUDE.md) to point at `start-services.sh` and explain the underlying build+up mechanism. Added an explicit "do not add direct host bind-mounts for template files" caution.
  - Updated the admin page description in [client/src/app/templates/page.tsx](../../client/src/app/templates/page.tsx) to reference `start-services.sh` instead of the deleted script.
