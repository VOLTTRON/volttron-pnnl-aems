...
interface TimeRangeSelectorProps {
  ...
  onApply: (fromDate: Date, toDate: Date | null, useCurrentTime: boolean) => void;
...
export function TimeRangeSelector({
  ...
  const [localUseCurrentTime, setLocalUseCurrentTime] = useState<boolean>(useCurrentTime);
  const [localSelectedPreset, setLocalSelectedPreset] = useState<string>(selectedPreset);
  const [validationError, setValidationError] = useState<string>("");
  
  // Applied state (what's currently active)
  const [appliedFromDate, setAppliedFromDate] = useState<Date>(fromDate);
  const [appliedToDate, setAppliedToDate] = useState<Date | null>(toDate);
  const [appliedUseCurrentTime, setAppliedUseCurrentTime] = useState<boolean>(useCurrentTime);
  const [appliedPreset, setAppliedPreset] = useState<string>(selectedPreset);

  // Update applied state when props change (from external refresh)
  useEffect(() => {
    setAppliedFromDate(fromDate);
    setAppliedToDate(toDate);
    setAppliedUseCurrentTime(useCurrentTime);
    setAppliedPreset(selectedPreset);
  }, [fromDate, toDate, useCurrentTime, selectedPreset]);
  ...
  const handleApply = () => {
    ...
    onApply(actualFromDate, localToDate, localUseCurrentTime);
    onPresetChange(localSelectedPreset);
  };

  const handleRefresh = () => {
    // Re-apply the current settings
    onApply(appliedFromDate, appliedToDate, appliedUseCurrentTime);
  };
