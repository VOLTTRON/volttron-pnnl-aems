...
import { ReadLogsQuery, LogType as LogTypeGql, StringFilterMode, ReadLogsDocument } from "@/graphql-codegen/graphql";
...
export default function Page() {
  ...
  const logs = useMemo(
    () =>
      filter(data?.readLogs ?? [], search, LogType.parse(search) ? ["type", "message"] : ["message"]).map((log) => {
        return {
          ...log,
          message: <div>{parse(ansiUp.ansi_to_html(log.message ?? ""))}</div>,
        };
      }),
    [data?.readLogs, search],
  );
          ...
          <div className={styles.table}>
            <Table
              ...
              columns={[
                ...
                { field: "message", label: "Message", type: "element" },
