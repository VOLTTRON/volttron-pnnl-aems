...
export function Occupancies({ unit, editing, handleChange, readOnly = false }: OccupanciesProps) {
  ...
  const handleRemoveOccupancy = useCallback(
    (index: number) => {
      const occupancies = getOccupancies();
    ...
    <div style={{ padding: "1rem" }}>
      {!readOnly && (
        <div style={{ marginBottom: "1rem" }}>
          <Button icon={IconNames.PLUS} intent={Intent.SUCCESS} onClick={() => setShowAddOccupancy(!showAddOccupancy)}>
      ...
      {showAddOccupancy && (
        <Card style={{ padding: "1rem", marginBottom: "1rem" }}>
          ...
          <div style={{ display: "flex", gap: "0.5rem", marginTop: "1rem" }}>
            ...
            <Button icon={IconNames.CROSS} onClick={() => setShowAddOccupancy(false)}>
              Cancel
            </Button>
          </div>
        </Card>
