...
const mockBook = {
  ...
  duration: "20h 56m",
  date: "June 15, 2011",
  chapters: [{ title: "Prologue", pages: 1 }, { title: "1: Holden", pages: 3 }],
};
...
describe("Demo [isbn]/[index] layout", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("calls addResolver with 'chapter' when chapter resolver is not registered", async () => {
    const addResolver = jest.fn();
    await act(async () => {
      renderLayout({}, addResolver);
    });
    expect(addResolver).toHaveBeenCalledWith("chapter", expect.any(Function));
  });

  it("does not call addResolver when 'chapter' is already in resolvers", async () => {
    const addResolver = jest.fn();
    await act(async () => {
      renderLayout({ chapter: jest.fn() }, addResolver);
    });
