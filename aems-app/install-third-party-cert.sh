#!/bin/sh
#
# Install a third-party TLS certificate into the AEMS Traefik proxy.
#
...
# -- pre-flight ---------------------------------------------------------------
if [ ! -f "$ENV_FILE" ]; then
  err "$ENV_FILE not found -- run from the aems-app/ repo root."
  exit 2
fi
if [ ! -f "$YAML_FILE" ]; then
  err "Expected file $YAML_FILE not found -- is this the AEMS repo?"
  exit 2
fi
