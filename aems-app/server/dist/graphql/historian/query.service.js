...
let HistorianQuery = class HistorianQuery {
    constructor(builder, historianService, historianObject) {
        ...
        builder.queryField("historianSiteAggregateUnit", (t) => t.field({
            description: "Aggregate a unit metric across all systems on a site into a single time series (two-level bucketing: per-topic bucket, then across-topic aggregation)",
            authScopes: { user: true },
            type: historianObject.HistorianTimeSeries,
            args: {
                campus: t.arg.string({ required: true }),
